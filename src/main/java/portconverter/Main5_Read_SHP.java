...
public class Main5_Read_SHP {
	...
	private void go(String[] args) throws IOException {
		...
		File file = new File(
				"C:\\_ref_countries\\ref-countries-2016-01m.shp\\CNTR_RG_01M_2016_4326.shp\\CNTR_RG_01M_2016_4326.shp");
		...
		try (FeatureIterator<SimpleFeature> features = collection.features()) {
			if (features.hasNext()) { // only one for now
				SimpleFeature feature = features.next();
				int n = feature.getAttributeCount();
				for (int i = 0; i < n; i++) {
					String o = feature.getAttribute(i).toString();
